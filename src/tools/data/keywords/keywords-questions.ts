...
export class GetKeywordQuestions extends BaseTool {
  registerTool(server: McpServer): void {
    server.registerTool(
      this.toolName('getKeywordQuestions'),
      {
        title: 'Keyword Questions',
        description: 'Data Tool: Retrieves a list of question-based keywords containing the seed keyword.',
        inputSchema: {
          source: z
            .string()
          ...
          'filter[intents]': z
            ...
            .refine((val?: string | null) => this.isValidCommaSeparatedList(INTENTS, val), {
              message: 'filter[intents] must be a comma-separated list of supported intents (I, N, T, C, L)',
            })
            .describe(
