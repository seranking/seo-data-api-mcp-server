    ...
    registerTool(server: McpServer): void {
        server.registerTool(
            "backlinksSummary",
            {
                ...
                description: "Fetch extended backlink statistics for one or multiple targets. Optionally builds an analysis prompt comparing your domain vs a competitor for a given keyword (v1/backlinks/summary)",
                inputSchema: {
                    target: z
                        .union([
                            ...
                            z.array(
                                z.string().min(1)
                            ).min(1, "targets array cannot be empty"),
                        ])
                        .describe(
