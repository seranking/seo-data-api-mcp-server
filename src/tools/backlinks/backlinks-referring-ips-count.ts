    ...
    registerTool(server: McpServer): void {
        server.registerTool(
            'getReferringIpsCount',
            {
                ...
                inputSchema: {
                    target: z.string().describe('Aim of the request: root domain, host, or URL. Use array for batching multiple targets via POST.'), // Note: Schema description updated but Zod type is string for simplicity. Batching handled if passed as distinct tool or string array?
            ...
            async (params) => {
                return this.makeGetRequest('/v1/backlinks/referring-ips/count', params);
            },
        );
    }
}
