    ...
    server.registerTool(
      "keywordsQuestions",
      {
        ...
        inputSchema: {
          ...
          sort: z.enum(["keyword", "volume", "cpc", "difficulty", "competition"]).optional()
          ...
          "filter[difficulty][from]": z.number().int().min(0).max(100).optional()
            .describe("Minimum keyword difficulty score (0-100)."),
          "filter[difficulty][to]": z.number().int().min(0).max(100).optional()
            .describe("Maximum keyword difficulty score (0-100)."),
          "filter[cpc][from]": z.number().min(0).optional()
            .describe("Minimum Cost Per Click."),
          "filter[cpc][to]": z.number().min(0).optional()
            .describe("Maximum Cost Per Click."),
          "filter[competition][from]": z.number().min(0).max(1).optional()
            .describe("Minimum competition score (0.0-1.0)."),
          "filter[competition][to]": z.number().min(0).max(1).optional()
          ...
          "filter[characters_count][from]": z.number().int().min(1).optional()
            .describe("Minimum character length of the keyword."),
          "filter[characters_count][to]": z.number().int().min(1).optional()
            .describe("Maximum character length of the keyword."),
          "filter[serp_features]": z
            .string()
            .optional()
            .refine((val?: string | null) => this.isValidCommaSeparatedList(SERP_FEATURE_CODES, val), {
              message:
      ...
      async (params) => this.makeGetRequest('/v1/keywords/questions', params)
    );
  }
}
