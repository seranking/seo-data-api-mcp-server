  ...
  registerTool(server: McpServer): void {
    server.registerTool(
      "keywordsExport",
      {
        ...
        description: "Bulk retrieve metrics for a list of keywords (v1/keywords/export, POST form-data; source in query)",
        inputSchema: {
          source: z.string().min(1, "source is required").max(2)
            .describe("Alpha-2 country code of the regional keyword database."),
          keywords: z.array(z.string().min(1)).min(1, "at least one keyword is required").max(5000, "maximum of 5000 keywords per call")
            .describe("For a single keyword, use one keywords[] parameter. Repeat the keywords[] to analyze multiple keywords. A maximum of 5,000 keywords can be submitted per call."),
          sort: z.enum(["volume", "cpc", "difficulty", "competition", "history_trend"]).optional()
            .default("cpc")
            .describe("The field by which the returned list of keywords should be sorted. Common sortable fields include volume, cpc, difficulty, competition."),
          ...
          cols: z
            ...
            .describe("A comma-separated list of specific response parameter names to include in the output for each keyword. If omitted, all available data points are returned."),
        }
      },
      async (params: any) => {
