      ...
      'domainKeywordsComparison',
      {
        ...
        inputSchema: {
          ...
          domain: z
            ...
            .describe(
              'The primary domain for the analysis. For a keyword gap analysis (`diff=1`), this will be the domain that has the keywords.',
            ),
          compare: z
            .string()
            .min(1, 'compare is required')
            .describe(
              'The secondary domain for comparison. For a keyword gap analysis (`diff=1`), this will be the domain that is missing the keywords.',
            ),
          type: z
            .enum(['organic', 'adv'])
            .optional()
          ...
          diff: z
            ...
            .default(0)
            .describe(
              'Controls the comparison mode. Use `0` to find keywords common to both domains. Use `1` for a keyword gap analysis to find keywords the `domain` ranks for, but the `compare` domain does not.',
            ),
          order_field: z
            .enum([
              'keyword',
