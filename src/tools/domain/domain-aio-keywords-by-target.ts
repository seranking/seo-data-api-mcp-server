...
export class DomainAioKeywordsByTarget extends BaseTool {
  registerTool(server: McpServer): void {
    server.registerTool(
      "domainAioKeywordsByTarget",
      {
        title: "AIO Keywords by Target",
        description: "Fetch AI Overviews domain keywords by target (v1/domain/aio/keywords-by-target)",
        inputSchema: {
          ...
          sort: z.enum(["volume", "type", "snippet_length"]).optional()
            .default("volume")
            .describe("The field to sort the results by. Available values: volume, type, snippet_length."),
      ...
      async (params) => this.makeGetRequest('/v1/domain/aio/keywords-by-target', params)
    );
  }
}
