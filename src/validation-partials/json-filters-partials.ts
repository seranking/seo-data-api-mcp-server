...
export const ExampleJsonFilter =
  '[[{"type":"contains","value":"seo"},{"type":"contains","value":"tools"}],[{"type":"contains","value":"backlinks"}]]';

export const InvalidFilterMessage =
  'Must be a URL-encoded JSON array of groups (each group is a non-empty array of clauses with type in begins|contains|ends|exact and non-empty value).';
...
export const FilterGroup = z.array(z.array(FilterClause).min(1)).min(1);
...
export const AISearchFilterObject = {
  ...
  filter_multi_keyword_excluded: z
    ...
    .optional()
    .describe(
      `A URL-encoded JSON string specifying keywords that must NOT be present in the prompt. For example: filter[multi_keyword_excluded]=${ExampleJsonFilter}`,
    )
    .refine(FilterGroupRefineCallback, { message: InvalidFilterMessage }),
};
